"""Tests for the composed processing pipeline (Stage 4, step 13).

The pipeline chains ``ExtractionService`` and ``NormalizationService`` against
one session. These run at the service layer (via ``db_session``) with a plain
callable extraction producer - no AI provider is involved. The per-stage
services are exercised directly too, to prove they stay independently callable.
"""
...
async def test_run_chains_extraction_then_normalization(db_session: AsyncSession) -> None:
...
async def test_normalization_failure_does_not_undo_the_extraction(
    ...
    monkeypatch.setattr(_ENGINE_PATH, engine_boom)
...
async def test_run_retry_reretries_extraction_then_normalizes(
